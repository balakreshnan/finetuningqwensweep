# Fine tuning Open Source model with Pytche - nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-BF16
...
# ─── Workaround: disable TRL chunked cross-entropy patching (incompatible with Nemotron) ───
import trl.trainer.sft_trainer as _sft_mod
_sft_mod._patch_chunked_ce_lm_head = lambda *args, **kwargs: None

# ─── Workaround: use base Trainer's compute_loss (TRL's version expects num_valid_tokens) ───
from transformers import Trainer as _BaseTrainer
SFTTrainer.compute_loss = _BaseTrainer.compute_loss
...
# ─── Push to HuggingFace Hub (only from rank 0, only if token is set) ───
if int(os.environ.get("LOCAL_RANK", 0)) == 0 and hf_token:
    print(f"Pushing model to HuggingFace: {hub_model_id}...")
    api = HfApi(token=hf_token)
    api.create_repo(hub_model_id, exist_ok=True, repo_type="model")
    api.upload_folder(folder_path=final_path, repo_id=hub_model_id, repo_type="model",
                      commit_message="Fine-tuned Nemotron-3.5-30B-A3B with LoRA on SQuAD")
    print(f"✅ Model uploaded to https://huggingface.co/{hub_model_id}")
else:
    print("Skipping Hub push (no HF_TOKEN or not rank 0). Model saved locally at:", final_path)
...
```
accelerate launch --num_processes=4 /workspace/finetune_lora.py
```

![nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-BF16 fine tuning](images/nemolightningpytcheft-1.png)

![nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-BF16 fine tuning](images/nemolightningpytcheft-2.png)

![nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-BF16 fine tuning](images/nemolightningpytcheft-3.png)

![nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-BF16 fine tuning](images/nemolightningpytcheft-4.png)

- upload to Huggingface

```
hf upload Balab2021/nemotron-3.5-30b-a3b-lora-finetuned /workspace/finetune-results/final_model . --commit-message "Fine-tuned Nemotron-3.5-30B-A3B with LoRA on SQuAD"
```

![nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-BF16 fine tuning](images/nemolightningpytcheft-5.png)
