...
def parse_args():
    ...
    parser.add_argument(
        "--push_to_hub",
        action="store_true",
        help="Upload the final LoRA adapter to Hugging Face Hub",
    )
...
def upload_adapter_to_hub(
    ...
    """Upload the saved LoRA adapter to a date-stamped Hugging Face repo."""

    if not hf_token:
        raise ValueError(
            "A Hugging Face token is required for upload. "
            "Use --hf_token or set HF_TOKEN."
        )
    ...
    repo_name = f"{base_model_name}-oasst1-lora_{date_suffix}"
    ...
    api.upload_folder(
        ...
        commit_message=(
            f"Upload QLoRA adapter trained from {model_name} "
            f"on OpenAssistant/oasst1"
        ),
    )
...
def main():
    ...
    if args.push_to_hub:
        hub_repo_id, hub_url = upload_adapter_to_hub(
    ...
    if hub_url:
        print(f"  Hugging Face repo : {hub_repo_id}")
        print(f"  Hugging Face URL  : {hub_url}")
