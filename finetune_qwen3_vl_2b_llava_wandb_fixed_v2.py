...
class MultimodalDataCollator:
    ...
    def __call__(
        ...
        # Do not truncate multimodal inputs here. Truncating after image
        # placeholders are expanded can remove part of an image-token block,
        # producing a mismatch between image tokens in the text and input_ids.
        #
        # Control memory with image resolution, micro-batch size, and dataset
        # filtering rather than processor-level truncation.
        ...
        if sequence_length > self.max_length:
            print(
                f"[warning] Processed multimodal batch length {sequence_length} "
                f"exceeds --max-length {self.max_length}. "
                "The batch is kept intact to preserve image-token alignment."
            )
