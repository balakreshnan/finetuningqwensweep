...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Run the four predefined experiments (default: enabled). "
            "Use --no-run_experiments for a single run."
        ),
    )
...
def write_experiment_csv(csv_path, results):
    """Write all results collected so far so metrics survive interruptions."""
    fieldnames = [
        "run", "status", "error", "adapter_path", "output_dir",
        "num_epochs", "learning_rate", "lora_r", "lora_alpha",
        "lora_dropout", "weight_decay", "train_loss", "eval_loss",
        "loss_gap", "train_runtime", "samples_per_second", "global_steps",
    ]
...
def run_experiment_suite(args, train_dataset, val_dataset):
    ...
    suite_dir = os.path.join(args.output_dir, f"experiments_{timestamp}")
    os.makedirs(suite_dir, exist_ok=True)

    csv_path = os.path.join(suite_dir, "experiment_results.csv")
    results = []
    write_experiment_csv(csv_path, results)

    print(f"  Experiment suite : {suite_dir}")
    print(f"  Live metrics CSV : {csv_path}")
    ...
    for index, config in enumerate(EXPERIMENTS, start=1):
        ...
        run_args.output_dir = os.path.join(suite_dir, config["name"])
        ...
        try:
            ...
            trainer, eval_result, metrics = train(
                model, tokenizer, train_dataset, val_dataset, run_args
            )

            # Persist metrics before adapter saving. If saving fails, the
            # completed training metrics are still retained in the CSV.
            row.update(metrics)
            row["status"] = "trained"
            write_experiment_csv(csv_path, results)

            adapter_path = save_adapter(
                trainer, tokenizer, run_args.output_dir
            )
            ...
            write_experiment_csv(csv_path, results)
            print(f"\n  Experiment interrupted. Partial results saved to: {csv_path}")
            raise
    ...
    if completed:
        for row in sorted(completed, key=lambda item: item["eval_loss"]):
            status = (
                "TARGET MET"
        ...
        best = min(candidates, key=lambda item: item["eval_loss"])
...
def main():
    ...
    print(
        "  Mode    : "
        + ("4-run experiment suite" if args.run_experiments else "single run")
    )
