...
def build_collator(processor: Any, max_length: int):
    tok = processor.tokenizer
    visual_ids = set()
    for special in ["<|image_pad|>", "<|video_pad|>", "<|vision_start|>", "<|vision_end|>", "<image>"]:
        tid = tok.convert_tokens_to_ids(special)
        if tid is not None and tid != tok.unk_token_id and tid >= 0:
            visual_ids.add(int(tid))

    def collate(examples: list[dict[str, Any]]) -> dict[str, torch.Tensor]:
        texts = [processor.apply_chat_template(x["messages"], tokenize=False, add_generation_prompt=False).strip() for x in examples]
        images = [normal_images(x) for x in examples]
        batch = processor(text=texts, images=images, return_tensors="pt", padding=True, truncation=True, max_length=max_length)
        labels = batch["input_ids"].clone()
        labels[labels == tok.pad_token_id] = -100
        for tid in visual_ids:
...
def main() -> None:
    ...
    try:
        ...
        trainer = SFTTrainer(
            ...
            data_collator=build_collator(processor, args.max_length),
